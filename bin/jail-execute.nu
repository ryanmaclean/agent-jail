...
#
# Canonical home: ryanmaclean/agent-jail (registry: "jail isolation -> agent-jail").
# Vendored from ryanmaclean/smolfire@ae2110b bin/jail-execute.nu per
# smolfire ADR 0001 / agent-jail#2. Behaviour-preserving: the pure helpers,
# run-jail-task, and the `run` CLI are unchanged. The smolfire-coordinator mbox
# protocol (`dispatch`, reply-envelope, mbox-append-prefix) is NOT here: that is
# the caller's envelope and stays in smolfire's thin adapter.
#
# Sibling of smolfire's bin/vm-execute.nu with the same contract: task id + commands in,
...
#
# This is the bounded experiment from smolfire docs/BOOT-TIME-ROADMAP.md §4
...
#   nu bin/jail-execute.nu contract        # machine-readable contract (JSON)
...
# Machine-readable description of this executor's contract, for agents that
# discover it cold (AX-first). Pure: no host access.
export def executor-contract [] {
    {
        schema: "agent-jail.executor.contract.v1"
        executor: "jail"
        executor_schema: $JAIL_EXECUTOR_SCHEMA
        canonical_repo: "ryanmaclean/agent-jail"
        entrypoint: {cli: "nu bin/jail-execute.nu run <task_id> <cmd>...", module: "use bin/jail-execute.nu [run-jail-task]"}
        backends: [
            {flag: "--base", kind: "nullfs", rootfs: "read-only nullfs mount of a host directory; tmpfs /tmp"}
            {flag: "--zfs-snapshot", kind: "zfs-clone", rootfs: "ephemeral writable zfs clone of DATASET@SNAP, destroyed on exit"}
            {flag: "--image", kind: "oci", rootfs: "OCI image via podman + ocijail"}
        ]
        result: {
            required: {verdict: "pass | fail", boot_sec: "int (jail setup seconds)", outputs: "list<{cmd, stdout, stderr, exit_code}>"}
            optional: {error: "string, present only when non-empty", warnings: "list<string>, present only when non-empty"}
        }
        exit_codes: {"0": "pass", "1": "fail", "2": "refused (non-FreeBSD host, or below patch floor without --allow-unpatched)"}
        limits: {
            default_timeout_sec: $DEFAULT_TIMEOUT_SEC
            coord_reply_window_sec: $COORD_REPLY_WINDOW_SEC
            teardown_reserve_sec: $TEARDOWN_RESERVE_SEC
            timeout_exit_code: $TIMEOUT_EXIT_CODE
            killed_exit_code: $KILLED_EXIT_CODE
        }
        network: {default: false, capability: $NETWORK_CAPABILITY}
        privilege: {escalation: "mdo(1) via mac_do(4)", never: ["sudo" "doas"]}
        host_requirements: {os: "freebsd", min_patch_level: $MIN_PATCH_LEVEL_MSG, rctl: "kern.racct.enable=1 (optional unless --require-limits)"}
        not_owned_here: ["scheduling/routing (smolfire coord-tick resolve-executor)", "mbox dispatch envelope (smolfire adapter)", "run identity (bop)"]
    }
}

# Print the executor contract as JSON.
def "main contract" [] {
    print (executor-contract | to json)
}
...
def main [] {
    ...
    print "  nu bin/jail-execute.nu contract    # JSON contract for agents"
}
