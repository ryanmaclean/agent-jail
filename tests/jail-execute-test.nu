...
# tests/jail-execute-test.nu — bin/jail-execute.nu (agent-jail canonical copy).
#
# Vendored from ryanmaclean/smolfire@ae2110b tests/jail-execute-test.nu (agent-jail#2).
# Test numbers are kept from the smolfire suite so results can be compared
# 1:1. Tests 10 and 22-27 exercise smolfire's coordinator (coord-tick.nu
# executor routing, mbox dispatch envelope) and stay in smolfire with its
# thin adapter; tests 35+ are agent-jail additions.
#
...
do {
    # smolfire's run-vm-task shape, pinned here (smolfire bin/vm-execute.nu:
    # {verdict, boot_sec, outputs, error?}) so parity is checked without a
    # cross-repo import. Its early-return error shape is
    # {verdict: "fail", boot_sec: 0, outputs: [], error: "..."}.
    let vm = {verdict: "fail", boot_sec: 0, outputs: [], error: "image not found: /nonexistent/smolfire-test.qcow2"}
...
# ── agent-jail additions ──────────────────────────────────────────────────────

print "test 35: executor contract is machine-readable and matches the constants"
do {
    let c = executor-contract
    assert equal $c.schema "agent-jail.executor.contract.v1"
    assert equal $c.executor "jail"
    assert equal $c.executor_schema $JAIL_EXECUTOR_SCHEMA
    assert equal $c.canonical_repo "ryanmaclean/agent-jail"
    assert equal ($c.backends | get flag) ["--base" "--zfs-snapshot" "--image"]
    assert equal ($c.result.required | columns) (result-record "pass" 0 [] | columns) "required keys = pass shape"
    assert equal $c.limits.default_timeout_sec $DEFAULT_TIMEOUT_SEC
    assert (($c.limits.default_timeout_sec + $c.limits.teardown_reserve_sec) <= $c.limits.coord_reply_window_sec) "contract fits reply window"
    assert equal $c.network.default false
    assert equal $c.network.capability $NETWORK_CAPABILITY
    assert ("sudo" in $c.privilege.never) "never sudo"
    # every documented exit code is one result-exit can actually return
    let refused = run-jail-task "t-c" ["true"] --base "/b" --os "macos"
    assert equal (result-exit $refused | into string) "2"
    assert ("2" in ($c.exit_codes | columns))
    assert equal (result-exit (result-record "pass" 0 [])) 0
    assert equal (result-exit (result-record "fail" 0 [] "boom")) 1
}

print "test 36: `contract` CLI prints parseable JSON; no smolfire dispatch subcommand"
do {
    let out = ^nu bin/jail-execute.nu contract | complete
    assert equal $out.exit_code 0
    let c = $out.stdout | from json
    assert equal $c.schema "agent-jail.executor.contract.v1"
    assert equal $c ((executor-contract) | to json | from json) "CLI == module"
    let usage = ^nu bin/jail-execute.nu | complete
    assert (not ($usage.stdout | str contains "dispatch")) "mbox dispatch stays in smolfire's adapter"
}
